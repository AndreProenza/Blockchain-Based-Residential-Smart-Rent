...
```bash
kubectl hlf inspect -c=unlockit --output resources/network.yaml -o Org1MSP -o Org2MSP -o OrdererMSP
```
...
export CHAINCODE_NAME=unlockit-chaincode
export CHAINCODE_LABEL=unlockit-chaincode
...
```bash
export IMAGE="unlockit/unlockit-smart-contract-ts:latest"
```
...
```bash
docker build -t $IMAGE --platform=linux/amd64 --file=./unlockit-smart-contract/Dockerfile ./unlockit-smart-contract
```

Otherwise, just run:

```bash
docker build -t $IMAGE --file=./unlockit-smart-contract/Dockerfile ./unlockit-smart-contract
```
...
kubectl hlf chaincode approveformyorg --config=resources/network.yaml --user=admin --peer=org1-peer0.default \
    --package-id=$PACKAGE_ID \
    --version "$VERSION" --sequence "$SEQUENCE" --name=unlockit-chaincode \
    --policy="AND('Org1MSP.member', 'Org2MSP.member')" --channel=unlockit
```
...
kubectl hlf chaincode invoke --config=resources/network.yaml \
    ...
    --chaincode=unlockit-chaincode --channel=unlockit \
    --fcn=InitLedger
```
